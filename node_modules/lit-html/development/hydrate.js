 ...
 */
import { noChange, _$private, } from './lit-html.js';
import { PartType } from './directive.js';
import { isPrimitive, isSingleExpression, isTemplateResult, } from './directive-helpers.js';
const { _TemplateInstance: TemplateInstance, _isIterable: isIterable, _resolveDirective: resolveDirective, _ChildPart: ChildPart, _EventPart: EventPart, _PropertyPart: PropertyPart, } = _$private;
/**
...
export const hydrate = (rootValue, container, options = {}) => {
    ...
    while ((marker = walker.nextNode()) !== null) {
        ...
        if (markerText.startsWith('lit-part')) {
            ...
            rootPart ?? (rootPart = currentChildPart);
        }
...
const openChildPart = (rootValue, marker, stack, options) => {
    ...
    if (value === noChange) {
        ...
        if (marker.data === markerWithDigest) {
            const template = ChildPart.prototype._$getTemplate(value.strings, value);
...
const createAttributeParts = (node, stack, options) => {
    ...
    if (state.type === 'template-instance') {
        ...
        while (true) {
            ...
            // template part
            const instancePart = new templatePart._constructor(node.parentElement, templatePart._name, templatePart._strings, state.instance, options);
            ...
            const noCommit = !(instancePart instanceof EventPart ||
